...
PHP_METHOD(Lxd_Lib_Certificate, generate) {
	...
	ZVAL_LONG(&_0, 275);
	ZEPHIR_CALL_FUNCTION(&_1, "filter_var", NULL, 27, &ip, &_0);
	...
	ZEPHIR_CALL_FUNCTION(&privkey, "openssl_pkey_new", NULL, 28, &_2);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_3);
	...
	ZEPHIR_CALL_FUNCTION(&cert, "openssl_csr_new", NULL, 29, &_3, &privkey);
	ZEPHIR_UNREF(&privkey);
	zephir_check_call_status();
	ZVAL_LONG(&_0, 1825);
	ZEPHIR_CALL_FUNCTION(&_4, "openssl_csr_sign", NULL, 30, &cert, &__$null, &privkey, &_0);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(NULL, "openssl_pkcs12_export", NULL, 33, &certString, &p12String, &privkeyString, &_6, &_5);
	ZEPHIR_UNREF(&p12String);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&pemString);
	ZEPHIR_CONCAT_VV(&pemString, &certString, &privkeyString);
	ZEPHIR_CALL_FUNCTION(&_7, "sha1", NULL, 34, &pemString);
	zephir_check_call_status();
	...
	if (!((zephir_file_exists(&cert_path TSRMLS_CC) == SUCCESS))) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "mkdir", NULL, 35, &cert_path, &_10$$4, &__$true);
		zephir_check_call_status();
	}
	ZEPHIR_INIT_VAR(&_11);
	...
	ZEPHIR_CALL_FUNCTION(&_15, "date_create", NULL, 36);
	zephir_check_call_status();
