...
PHP_METHOD(Lxd_Lib_Certificate, generate) {
	...
	ZEPHIR_CALL_FUNCTION(&privkey, "openssl_pkey_new", NULL, 15, &_0);
	...
	ZEPHIR_CALL_FUNCTION(&cert, "openssl_csr_new", NULL, 16, &_1, &privkey);
	ZEPHIR_UNREF(&privkey);
	zephir_check_call_status();
	ZVAL_LONG(&_2, 1825);
	ZEPHIR_CALL_FUNCTION(&_3, "openssl_csr_sign", NULL, 17, &cert, &__$null, &privkey, &_2);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(&cert, &_3);
	ZEPHIR_MAKE_REF(&certString);
	ZEPHIR_CALL_FUNCTION(NULL, "openssl_x509_export", NULL, 18, &cert, &certString);
	ZEPHIR_UNREF(&certString);
	zephir_check_call_status();
	ZEPHIR_MAKE_REF(&privkeyString);
	ZEPHIR_CALL_FUNCTION(NULL, "openssl_pkey_export", NULL, 19, &privkey, &privkeyString);
	...
	ZEPHIR_CALL_FUNCTION(NULL, "openssl_pkcs12_export", NULL, 20, &certString, &p12String, &privkeyString, &_5, &_4);
	...
	ZEPHIR_INIT_NVAR(&_5);
	ZVAL_STRING(&_5, "sha1");
	ZEPHIR_CALL_FUNCTION(&_3, "hash", NULL, 21, &_5, &pemString);
	zephir_check_call_status();
	...
	if (!((zephir_file_exists(&cert_path TSRMLS_CC) == SUCCESS))) {
		ZVAL_LONG(&_8$$4, 0755);
		ZEPHIR_CALL_FUNCTION(NULL, "mkdir", NULL, 22, &cert_path, &_8$$4, &__$true);
		zephir_check_call_status();
	}
	ZEPHIR_INIT_VAR(&_9);
	...
	ZEPHIR_CALL_FUNCTION(&_13, "date_create", NULL, 23);
	zephir_check_call_status();
